...
func (s *glShader) createShader(name, header, opts, vShader, fShader string) error {
	...
	vertexShader := Shader{Value: gl.CreateShader(gl.VERTEX_SHADER)}
	ShaderSource(vertexShader, strings.Join([]string{header, opts, vShader}, "\n"))
	gl.CompileShader(vertexShader.Value)
	status := Enum(GetShaderi(vertexShader, gl.COMPILE_STATUS))
	if status != gl.TRUE {
		return dumpShaderError(vertexShader, name, "vert")
	}

	fragmentShader := Shader{Value: gl.CreateShader(gl.FRAGMENT_SHADER)}
	ShaderSource(fragmentShader, strings.Join([]string{header, opts, fShader}, "\n"))
	gl.CompileShader(fragmentShader.Value)
	...
	if status != gl.TRUE {
		return dumpProgramError(Program{Value: program}, name)
	}

	s.vertexAttrib = Attrib{Value: uint(gl.GetAttribLocation(program, gl.Str("vertex"+"\x00")))}
	s.tcoordAttrib = Attrib{Value: uint(gl.GetAttribLocation(program, gl.Str("tcoord"+"\x00")))}

	s.program = Program{Value: program}
...
func (s *glShader) deleteShader() {
	if s.program.Valid() {
		gl.DeleteProgram(s.program.Value)
	}
	if s.vertex.Valid() {
		gl.DeleteShader(s.vertex.Value)
	}
	if s.fragment.Valid() {
		gl.DeleteShader(s.fragment.Value)
	}
}
...
func (p *glParams) renderCreateTexture(texType nvgTextureType, w, h int, flags ImageFlags, data []byte) int {
	p.context.checkError("create tex bef")
	...
	p.context.checkError("create tex -1")
	...
	if texType == nvgTextureRGBA {
		...
		p.context.checkError("create tex ld")

	} else {
		p.context.checkError("create tex booo")

		data = prepareTextureBuffer(data, w, h, 1)
		TexImage2D(gl.TEXTURE_2D, 0, w, h, gl.RED, gl.UNSIGNED_BYTE, data)
		p.context.checkError("create tex ooo")

	}
	p.context.checkError("create tex 0")
...
func (p *glParams) renderFlush() {
	...
	if len(c.calls) > 0 {
		gl.UseProgram(c.shader.program.Value)
		...
		gl.BindBuffer(gl.ARRAY_BUFFER, c.vertexBuffer.Value)
		BufferData(gl.ARRAY_BUFFER, b, gl.STREAM_DRAW)
		gl.EnableVertexAttribArray(uint32(c.shader.vertexAttrib.Value))

		gl.EnableVertexAttribArray(uint32(c.shader.tcoordAttrib.Value))

		gl.VertexAttribPointer(uint32(c.shader.vertexAttrib.Value), 2, gl.FLOAT, false, 4*4, gl.PtrOffset(0))
		gl.VertexAttribPointer(uint32(c.shader.tcoordAttrib.Value), 2, gl.FLOAT, false, 4*4, gl.PtrOffset(8))

		// Set view and texture just once per frame.
		gl.Uniform1i(c.shader.locations[glnvgLocTEX].Value, 0)
		...
		gl.DisableVertexAttribArray(uint32(c.shader.vertexAttrib.Value))
		gl.DisableVertexAttribArray(uint32(c.shader.tcoordAttrib.Value))
		gl.Disable(gl.CULL_FACE)
